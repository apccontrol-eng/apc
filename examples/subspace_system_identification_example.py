...
# --------------------------------------------------
# simulating deterministic/nominal model
x = np.zeros((N, n))
y = np.zeros((N, p))
for k in range(N - 1):
    y[k] = C @ x[k] + D @ u[k]
...
# --------------------------------------------------
# identification
A_id, B_id, C_id, D_id = MOESP(u_c, y_c, i=10, n=2)
...
# --------------------------------------------------
# intuitively if some entry of any of the identified matrices is 
# very very small we can almost assume it is zero and the smallness of the 
# entry is just due to minimizing MSE in the presence of Gaussian noise 
